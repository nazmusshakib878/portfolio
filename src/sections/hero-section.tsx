...
export function HeroSection() {
  ...
  return (
    <section id="hero" className="relative scroll-mt-[7.5rem] lg:scroll-mt-40 overflow-hidden rounded-[2rem] border border-border/60 bg-[linear-gradient(135deg,rgba(255,255,255,0.96),rgba(248,250,252,0.9))] px-6 py-16 shadow-[0_30px_120px_rgba(15,23,42,0.08)] backdrop-blur-sm sm:px-10 sm:py-20 lg:px-16 lg:py-24 dark:bg-[linear-gradient(135deg,rgba(15,23,42,0.95),rgba(17,24,39,0.9))]">
      ...
      <div className="relative grid gap-12 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
        <motion.div
          ...
          className="space-y-8"
        >
          <Badge variant="outline" className="rounded-full border-sky-300/60 bg-white/70 px-3 py-1 text-foreground shadow-sm backdrop-blur-md dark:bg-slate-900/60">
          ...
          <div className="flex flex-wrap gap-3">
            <Button size="lg" className="rounded-full px-5" render={<a href="#projects" />}>
              View Projects
              <MoveRight />
            </Button>
            <Button variant="outline" size="lg" className="rounded-full border-border/70 bg-background/70 px-5 backdrop-blur-md" render={<a href="#contact" />}>
              Contact Me
            </Button>
          </div>

          <div className="flex flex-wrap items-center gap-3">
            <LinkButton href={portfolioData.githubUrl} className="rounded-full border border-border/60 bg-background/70 px-4 text-muted-foreground hover:text-foreground">
              <Code2 />
              GitHub
            </LinkButton>
            {portfolioData.linkedinUrl ? (
              <LinkButton href={portfolioData.linkedinUrl} className="rounded-full border border-border/60 bg-background/70 px-4 text-muted-foreground hover:text-foreground">
                <BriefcaseBusiness />
                LinkedIn
              </LinkButton>
            ) : null}
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            {portfolioData.heroStats.map((stat) => (
              <div key={stat.label} className="rounded-2xl border border-border/60 bg-white/70 p-4 shadow-sm backdrop-blur-md dark:bg-slate-900/60">
            ...
            <div className="relative space-y-5">
              <div className="rounded-2xl border border-sky-200/80 bg-gradient-to-br from-sky-50 to-violet-50 p-5 shadow-sm dark:border-slate-800 dark:from-slate-900/80 dark:to-slate-950/70">
                <div className="mb-4 flex items-center gap-3">
                  ...
                  <span className="text-sm text-muted-foreground">Premium UI systems and production-ready frontends</span>
                </div>
                <div className="space-y-3">
                  <div className="h-3 w-5/6 rounded-full bg-foreground/15" />
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {portfolioData.heroBadges.map((badge) => (
                  <div key={badge.label} className="rounded-2xl border border-border/60 bg-white/80 px-4 py-3 text-sm font-medium text-foreground shadow-sm backdrop-blur-md dark:bg-slate-900/70">
                    {badge.label}
                  </div>
                ))}
              </div>
              ...
              <div className="rounded-2xl border border-border/60 bg-background/80 p-4 shadow-sm backdrop-blur-md">
                <p className="text-sm leading-7 text-muted-foreground">{portfolioData.bio}</p>
              </div>
            </div>
          </div>
        </motion.div>
